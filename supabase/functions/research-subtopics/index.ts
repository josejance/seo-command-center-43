...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const subTopics = task?.result?.[0]?.sub_topics || [];

    const keywords = subTopics.map((topic: any) => ({
      keyword: typeof topic === "string" ? topic : topic.topic || topic.sub_topic || topic.title || JSON.stringify(topic),
